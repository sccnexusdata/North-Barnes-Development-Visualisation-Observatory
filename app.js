const visualRules = {
  ...
  3:{detail:'A deliberately modest first residential stage. This is a modelling assumption, not a published construction programme.',show:['enabling','phase1']},
  ...
  10:{detail:'Roughly half of the illustrative housing scenario is present. Commercial and employment quantities remain scenario values; sports provision is supported in principle but its timing and geometry here remain illustrative.',show:['enabling','phase1','phase2','phase3','school','centre','employment','sports']},
  15:{detail:'Additional illustrative neighbourhoods fill out the internal structure while landscape elements mature. Named landscape components are supported in principle, not fixed to this schematic geometry.',show:['enabling','phase1','phase2','phase3','phase4','school','centre','employment','sports','green']},
  20:{detail:'Most illustrative housing is present. This is a placemaking/maturation scenario and does not constitute a published construction programme.',show:['enabling','phase1','phase2','phase3','phase4','phase5','school','centre','employment','sports','green']},
  25:{detail:'Illustrative mature scenario at the current maximum of 3,000 homes. Healthcare, Bevern Valley Park, sports hub/pavilion and Wet Home Wood nature reserve are supported components in principle; placement and timing remain illustrative. Detailed 6,000 m² and 10,000 m² quantities remain pending direct primary confirmation.',show:['enabling','phase1','phase2','phase3','phase4','phase5','phase6','school','centre','employment','sports','green','health']}
};

const stageLabels={
  enabling:'Illustrative enabling access or infrastructure corridor',
  phase1:'Illustrative residential neighbourhood stage one',
  phase2:'Illustrative residential neighbourhood stage two',
  phase3:'Illustrative residential neighbourhood stage three',
  phase4:'Illustrative residential neighbourhood stage four',
  phase5:'Illustrative residential neighbourhood stage five',
  phase6:'Illustrative residential neighbourhood stage six',
  school:'Illustrative school/community location; facility supported in principle, location not fixed',
  centre:'Illustrative local-centre location; detailed quantity and position not fixed',
  employment:'Illustrative employment location; detailed quantity and position not fixed',
  sports:'Illustrative sports location; sports provision supported in principle, location not fixed',
  health:'Illustrative healthcare location; healthcare supported in principle, location not fixed',
  green:'Illustrative maturing green-space representation; exact landscape geometry not fixed'
};

const fallbackPhases = [0,1,3,5,10,15,20,25].map(year => ({
  year,
  label: ({0:'Existing landscape',1:'Illustrative enabling stage',3:'Illustrative first neighbourhood',5:'Illustrative early community',10:'Illustrative settlement growth',15:'Illustrative maturation',20:'Illustrative late placemaking',25:'Illustrative mature scenario'})[year],
...
async function initTimelapse(){
  ...
  slider.setAttribute('aria-describedby','phase-model-note');
  const modelNote=root.querySelector('.model-note');
  if(modelNote) modelNote.id='phase-model-note';

  stageGroups.forEach(group=>{
    group.setAttribute('role','group');
    group.setAttribute('aria-label',stageLabels[group.dataset.stage]||'Illustrative scenario element');
  });
  ...
  function render(index,announce=false){
    ...
    if(announce && statusEl) statusEl.textContent=`${yearEl.textContent}: ${phase.label}. ${Number(phase.illustrative_homes||0).toLocaleString('en-GB')} illustrative homes in the scenario.`;
  }

  function stop(){
    if(timer) clearInterval(timer);
    timer=null;
    play.textContent='Play scenario';
    play.setAttribute('aria-pressed','false');
  }

  function start(){
    stop();
    play.textContent='Pause scenario';
    play.setAttribute('aria-pressed','true');
    timer=setInterval(()=>{
      let next=Number(slider.value)+1;
      if(next>=phases.length) next=0;
      render(next,true);
    },reduceMotion?2800:1700);
  }
...
function loadProposalOverlay(){
  ...
  script.src='proposal-overlay.js?v=19';
...
function installViewerViewportGuard(){
  ...
  document.addEventListener('click',event=>{
    if(event.target.closest?.('[data-enter],[data-view],[data-fullscreen],[data-proposal-toggle],[data-proposal-phase]')){
